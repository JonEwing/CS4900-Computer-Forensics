...
FILE *ptr;
	...
	{	
		...
		{
			ext2_in = get_inode(inodes[i],block_size,inode_size); //Get Inode 
			size = ext2_in.i_size;
			
			cout<< "inode # "<<inodes[i]<<endl;
			for(int j = 0; j < 12; j++)
			{
				unsigned int i_offset = ext2_in.i_block[j] * block_size;
				fseek(ptr, i_offset, SEEK_SET);
				fread(bbuf, block_size, 1, ptr);
		...
		{
			ext2_in = get_inode(i,block_size,inode_size); //Get Inode 
			
			cout<<endl<< "inode # "<<i<<endl;
			for(int j = 0; j < 12; j++)
			{
				unsigned int i_offset = ext2_in.i_block[j] * block_size;
				fseek(ptr, i_offset, SEEK_SET);
